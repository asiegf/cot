...
(deftest deftestgen-defines-expected-tests
  (let [names (test-var-names)]
    ...
    (is (contains? names 'test-get-empty-items))
    (is (contains? names 'test-get-featured-items))
    (is (contains? names 'test-get-profile))
    (is (contains? names 'test-get-profiles))
...
(deftest inline-response-nested-ref-validation
  (testing "inline response object: :cot.schema/owner is registered and actually checks nested values"
    (is (true?  (s/valid? (s/keys :req-un [:cot.schema/owner])
                          {:owner {:id 1 :address {:city "NYC"}}})))
    (is (false? (s/valid? (s/keys :req-un [:cot.schema/owner])
                          {:owner {:id "not-an-int"}})))
    (is (false? (s/valid? (s/keys :req-un [:cot.schema/owner])
                          {:owner {:id 1 :address {:city 99}}}))))
  (testing "array of inline objects: each item is validated through the nested ref"
    (let [items-spec (s/coll-of (s/keys :req-un [:cot.schema/owner]) :min-count 1)]
